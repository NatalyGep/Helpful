...
``` bash
git config --system                            # показать список системных настроек
git config --list                              # показать список глобальных настроек
git config --local                             # показать список локальных настроек
git config --global user.name 'Имя Фамилия'    # глобально задать имя пользователя
git config --global user.email адрес почты     # глобально задать почтовый адрес
git config --global core.editor редактор       # глобально задать редактор
git help                                       # получить справку
git help команда                               # получить справкупо команде
git название_редактора                         # открыть файл в редакторе
```
...
ls ~/build                  # показать содержимое папки build (не текущая)
start .                     # открыть в проводнике текущую папку
start ~/d/projects/text.txt # открыть в проводнике указанную папку или файл     
```
...
git diff b053 c0u6      # сравнить два коммита (пишем первые 4 символа)
git diff --color-words  # сравнить рабочую директорию и индекс, показать отличия в словах (неотслеживаемые файлы ИГНОРИРУЮТСЯ)
git diff index.html     # сравнить файл из рабочей директории и индекс
git diff HEAD           # сравнить рабочую директорию и коммит, на который указывает HEAD (неотслеживаемые файлы ИГНОРИРУЮТСЯ)
git diff --staged       # сравнить индекс и коммит с HEAD (в области подготовленных файлов)
...
``` bash
git add .        # добавить в индекс все новые, изменённые, удалённые файлы из текущей директории и её поддиректорий работает так же git add -А
...
git reset HEAD путь_к_файлу/имя_файла     # отменить добавление файла в индекс
...
git commit -a -m "Name of commit" # проиндексировать отслеживаемые файлы (ТОЛЬКО отслеживаемые, но НЕ новые файлы) и закоммитить, добавить сообщение работает так же git commit -am "Name of commit"
```
...
git log --stat             # показать последние коммитs и статистику внесенных ими изменений
...
git reset HEAD text.txt # отменить индексацию изменений в файле (не хотим включать в комми
```
